...
export default function Round5Page() {
  ...
  const handleVideoEnd = () => {
    ...
    else if (stage === "postStats") setStage("win");
  };
  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center px-4 py-10">
      ...
      <div className="relative z-10 w-full max-w-screen-xl px-8 pt-8">
        {/* Top Row: Video and Sidebar */}
        <div className="flex justify-between gap-8">
          {/* Video (Top Left) */}
          <div className="w-2/3">
            {(stage === "intro" || stage === "win") && (
              <AlexVideoPlayer
                src={stage === "intro" ? "/video/round5-intro.mp4" : "/video/round5-win.mp4"}
        ...
        {stage === "question" && (
          <div className="mt-10 w-2/3">
            <h2 className="text-white text-xl font-bold mb-2">QUESTION 5</h2>
            <p className="text-white text-lg mb-4">{QUESTION_5.question}</p>
            <div className="grid grid-cols-2 gap-4">
              {QUESTION_5.options.map((opt, idx) => (
                <Button
                  ...
                  disabled={lockOptions || opt !== QUESTION_5.correctAnswer}
