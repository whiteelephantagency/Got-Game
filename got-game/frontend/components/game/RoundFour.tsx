...
export default function Round4Page() {
  ...
  // Stats animation effects + AUDIO (whoosh -> loop during fill -> chime)
  useEffect(() => {
    let interval: ReturnType<typeof setInterval> | null = null;
  ...
  // Flashing effect for finalist status (kept)
  useEffect(() => {
    let flashInterval: ReturnType<typeof setInterval> | null = null;
    if (finalistFlash) {
      flashInterval = setInterval(() => {
        // noop, just keep CSS animations lively
      }, 500);
    }
  ...
  const handleAnswer = (option: string) => {
    primeAudio(); // ensure audio can play later
  ...
  return (
    ...
    >
      {/* Hidden audio (kept off-screen so iOS preloads) */}
